#!/usr/bin/env python3
"""
遍历指定目录，生成 JSON 数据文件供 server.py 后端加载。
输出数据结构: { images: [...], folders: [...], mtime: ..., _stats: {...} }
...
def make_source_entry(path):
    """从路径推导 (name, output_json)"""
    dir_name = os.path.basename(os.path.normpath(path))
    json_name = f"{dir_name.lower()}.json"
    return (path, json_name)
...
def build_tree(dirpath, rel_path="", cache=None):
    ...
    if old_node and old_node.get('mtime') == current_mtime:
        # 复用缓存图片，但确保 URL 只存文件名
        cached_images = old_node.get('images', [])
        images = [{"name": img["name"], "url": os.path.basename(img["url"])} for img in cached_images]
        ...
        # URL 只存文件名（前端在当前目录上下文中使用）
        images = [{"name": f, "url": f} for f in files]

        folders = []
        for d in dirs:
            dir_full = os.path.join(dirpath, d)
            sub_rel = os.path.join(rel_path, d) if rel_path else d
            # 文件夹 URL 也只存文件夹名（前端在当前目录上下文中使用）
            dir_url = d
            child_data = build_tree(dir_full, sub_rel, cache=cache)
            folders.append({"name": d, "url": dir_url, **child_data})
...
def load_tree_cache(output_file):
    """从 JSON 文件中读取带 mtime 的树结构作为缓存"""
    if not os.path.isfile(output_file):
        return None
    try:
        with open(output_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
        # 返回树节点数据（去掉 _stats 等元信息）
        return {

...
| 功能 | 说明 |
|------|------|
| 目录树导航 | 左侧侧边栏，递归加载子目录，支持展开/折叠；无子文件夹时不显示展开箭头 |
...
| 视频播放 | 内联播放 + 进度条 + 静音切换 |
| 图片视频联动 | 同名 `.mp4` 文件自动关联图片，主窗口/全屏先显示图片，视频就绪后自动替换播放 |
| 全屏查看 | 双击进入，支持浏览器全屏 API |
| 缩略图面板 | 右侧显示当前图片组内所有缩略图，有匹配视频的 mp4 自动隐藏 |
...
当同组内存在与图片**严格同名**的 `.mp4` 文件时（仅扩展名不同），自动触发联动效果：

- **缩略图面板**：只展示图片缩略图，mp4 缩略图自动隐藏
- **主窗口**：先展示图片，同时在后台预加载 mp4；视频就绪后自动替换图片并静音循环播放
- **全屏模式**：同上，先图后视频自动切换
- **组内导航**：已被图片替代的 mp4 在上下键切换中跳过

> 匹配规则严格按完整文件名：`photo.jpg` → `photo.mp4`；`photo.bikini.jpg` → `photo.bikini.mp4`（不会匹配 `photo.mp4`）。
